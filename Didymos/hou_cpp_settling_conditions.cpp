...
struct parameters{
	...
	double* rho_pc;//density percentage
	int* rho_distribution;//density distribution
...
struct initialization{
	...
	double rho_pc;//density percentage
	double r_core;//core radius fraction [0,1]
	int rho_distribution;//density distribution
...
void poly_inertia_met_core(int q, double rho, string tet_file, string vert_file, cube* T, double rho_pc, double r_core);
void inertia_rot(mat C, int q, cube* T, cube* Tp);
void poly_moi_core(double rho, string tet_file, string vert_file, mat* I, double rho_pc, double r_core);
void poly_moi_met(double rho, string tet_file, string vert_file, mat* I);
void poly_moi_met_core(double rho, string tet_file, string vert_file, mat* I, double rho_pc, double r_core);
void ell_mass_params_met(double order, double order_body,double rho, double a, double b, double c, mat* I, cube* T);
void ell_mass_params_met_linear(double order, double order_body, double rho, double rho_pc, double a, double b, double c, mat* I, cube* T);
void ell_mass_params_met_core(double order, double order_body, double rho, double rho_pc, double r_core, double a, double b, double c, mat* I, cube* T);
...
double rk4_stack(double t0, double tf, mat x0, double h, int a_shape, parameters inputs, mat* tout, mat* xout, mat* hout, mat* sunout, std::function<mat(mat,mat, parameters)> ode);
double ABM(double t0, double tf, mat x0, double h, int a_shape, parameters inputs, mat* tout, mat* xout, mat* hout, mat*sunout, std::function<mat(mat,mat, parameters)> ode);
double LGVI_integ(double t0, double tf, mat x0, double h, parameters inputs, mat* tout, mat* xout);
...
double orbital_period(double guess);
double iterate_density_secant(double guess, double tol, int max_iterations, double orbital_period_observed);

int main() {
    cout << "IC read " << endl;
	ic_read(&ics);
	double guess = ics.rhoA;
    double tol = 1e-5;
    int max_iterations = 10;
    double orbital_period_observed = 11.9216289 * 3600;
    cout << "Running secant " << endl;
    double density = iterate_density_secant(guess, tol, max_iterations, orbital_period_observed);
    cout << "Density " << density << endl;
	return 0;
}

double iterate_density_secant(double guess, double tol, int max_iterations, double orbital_period_observed) {
	mat DP(2, max_iterations+2);
	int DPid = 0;
	double density_prev = guess;
	double density = density_prev*1.01;
	cout << "Guess 1 density " << density_prev << endl;
	cout << "Guess 2 density " << density << endl;
	double orbital_period_prev = orbital_period(density_prev);
	cout << "Guess 1 density " << density_prev << endl;
	cout << "Guess 1 period " << orbital_period_prev  << endl;
	(DP)(0, DPid) = density_prev;
	(DP)(1, DPid) = orbital_period_prev;
	DPid += 1;
	cout << DPid << endl;
	cout << DP << endl;

	if (std::abs(orbital_period_prev - orbital_period_observed) < tol) {
		DP.save("DP.csv", csv_ascii);
		cout << "Stop reason: Tolerance below value requested" << endl;
		return density_prev;
	}
	double orbital_period_now = orbital_period(density);
	(DP)(0, DPid) = density;
	(DP)(1, DPid) = orbital_period_now;
	DPid += 1;
	cout << DPid << endl;
	cout << DP << endl;

	if (std::abs(orbital_period_now - orbital_period_observed) < tol) {
		DP.save("DP.csv", csv_ascii);
		cout << "Stop reason: Tolerance below value requested" << endl;
		return density;
	}
	cout << "Guess 2 density" << density << endl;
	cout << "Guess 2 preiod" << orbital_period_now << endl;
	double orbital_period_next, density_next, f_prev, f_now;
	for (int i = 0; i < max_iterations; ++i) {
		// Update density estimate using the secant method
		f_prev = orbital_period_prev - orbital_period_observed;
		f_now = orbital_period_now - orbital_period_observed;
		density_next = density - ((density - density_prev) / (f_now - f_prev)) * f_now;
		std::cout << "Density iteration " << i << ": " << density_next << std::endl;
		orbital_period_next = orbital_period(density_next);
		(DP)(0, DPid) = density_next;
		(DP)(1, DPid) = orbital_period_next;
		DPid += 1;
		cout << DPid << endl;
		cout << DP << endl;

		std::cout << "Period iteration " << i << ": " << orbital_period_next << std::endl;

		// Check convergence
		if (std::abs(orbital_period_next - orbital_period_observed) < tol) {
			DP.save("DP.csv", csv_ascii);
			cout << "Stop reason: Tolerance below value requested" << endl;
			return density_next;
		}
		...
		orbital_period_prev = orbital_period_now;
		density = density_next;
		orbital_period_now = orbital_period_next;

		std::cout << "Guess 1 density: " << density_prev << std::endl;
		std::cout << "Guess 1 period: " << orbital_period_prev << std::endl;
		std::cout << "Guess 2 density: " << density << std::endl;
		std::cout << "Guess 2 period: " << orbital_period_now << std::endl;
	}
	DP.save("DP.csv", csv_ascii);
	cout << "Stop reason: maximum iterations reached" << endl;
	cout << "Tolerance = " << orbital_period_next - orbital_period_observed << endl;
	// Return the last computed density
	return density;
}

double orbital_period(double guess) {
	cout.precision(16);
	int order = ics.order;  // mutual potential truncation order
	int order_a = ics.order_a;   // primary shape order truncation
	int order_b = ics.order_b;   // secondary shape order truncation
	...
	if (ics.Tgen) {//generates inertia integrals if flag is set to 1
		...
		if (ics.a_shape == 0) {//sphere
			...
			// if (density_distribution == 0) {
				ell_mass_params_met(order, order_a, rhoA, ics.aA, ics.bA, ics.cA, &IA, &TA);
				rhoB = guess;
			// }
			// else if (density_distribution == 1) {
			// 	ell_mass_params_met_linear(order, order_a, rhoA, ics.rho_pc, ics.aA, ics.bA, ics.cA, &IA, &TA);
			// 	rhoB = (TA)(0, 0, 0) * 3 / (4 * M_PI*ics.aA/1000*ics.bA/1000*ics.cA/1000);
			// }
			// else if (density_distribution == 2) {
			// 	ell_mass_params_met_core(order, order_a, rhoA, ics.rho_pc, ics.r_core, ics.aA,  ics.bA, ics.cA, &IA, &TA);
			// 	rhoB = (TA)(0, 0, 0) * 3 / (4 * M_PI*ics.aA / 1000 * ics.bA / 1000 * ics.cA / 1000);
			// }
			...
			// }
			// else if (density_distribution == 2) {
			// 	cout << ics.r_core << endl;
			// 	poly_inertia_met_core(order_a, rhoA, tet_fileA, vert_fileA, &TA, ics.rho_pc, ics.r_core);
			// 	poly_moi_met_core(rhoA, tet_fileA, vert_fileA, &IA, ics.rho_pc, ics.r_core);
			// 	rhoB = (TA)(0, 0, 0) * 3 / (4 * M_PI*ics.aA / 1000 * ics.bA / 1000 * ics.cA / 1000);
			// 	cout << "rhoB " << rhoB;
			// }
	...
	IA.load("IDP.mat");
	IB.load("IDS.mat");
	TS.zeros(size(TA));  // Sphere inertia integral
	Tsun.zeros(size(TA));
	TS(0, 0, 0) = Mplanet;  // Mass of perturbing 3rd body
	Tsun(0, 0, 0) = Msolar;
	// calculate expansion coefficients
	tk_calc(order, &tk);  // Compute tk expansion coefficients [Eq. 9 Hou et al. 2016]
	a_calc(order, &a);  //  Compute the a expansion coefficients [Eq. 13 Hou et al. 2016]
	b_calc(order, &b);  //  Compute the b expansion coefficients [Eq. 13 Hou et al. 2016]
	...
	if (ics.integ == 1) {
		...
		period = rk4_stack(t0, tf, x0, ics.h, ics.a_shape, inputs, &t_sol, &x_sol, &h_sol, &sun_sol, hou_ode);
		cout << "Period " << period << endl;
	}
	else if (ics.integ == 2) {
		cout << "LGVI" << endl;//LGVI integrator
		period = LGVI_integ(t0, tf, x0, ics.h, inputs, &t_sol, &x_sol);
		cout << "Period " << period << endl;
	}
	else if (ics.integ == 3) {
		...
		period = ABM(t0, tf, x0, ics.h, ics.a_shape, inputs, &t_sol, &x_sol, &h_sol, &sun_sol, hou_ode);
	}
...
void poly_inertia_met_core(int q, double rho, string tet_file, string vert_file, cube* T, double rho_pc, double r_core){
	mat tet,vert,x1,x2,x3,x,X, X1, X2, X3;
	double Ta, TA, T1, T2;
	(*T).zeros(q+1,q+1,q+1);
    tet.load(tet_file,csv_ascii);
    tet-=1;//accounts for indexing from 0
        ...
        for(double m=0;m<(q+1-l);m++){
            for(double n=0;n<(q+1-l-m);n++){
                for(int a=0;a<tet.n_rows;a++){
					...
					X = r_core*x;
					TA = abs(det(X));
					T1  = rho_pc*rho * TA*tet_sums(l, m, n, X1(0, 0), X2(0, 0), X3(0, 0), X1(0, 1), X2(0, 1), X3(0, 1), X1(0, 2), X2(0, 2), X3(0, 2));
					T2 = T1 / rho_pc;
					(*T)(l, m, n) += rho_pc*rho * Ta*tet_sums(l, m, n, x1(0, 0), x2(0, 0), x3(0, 0), x1(0, 1), x2(0, 1), x3(0, 1), x1(0, 2), x2(0, 2), x3(0, 2)) - T1 + T2;
                }}}}
    return;
}
...
void poly_moi_core(double rho, string tet_file, string vert_file, mat* I, double rho_pc, double r_core){
    ...
    tet-=1;//accounts for indexing from 0
    vert.load(vert_file,csv_ascii);
    double V, VV, I1, I2, I3;
    mat p1,p2,p3,p4,p, P, P1, P2, P3, P4;
	rho_pc = rho_pc / 100;
    for(int a=0;a<tet.n_rows;a++){
        p1.zeros(1,3);//assuming all tet have vertex at origin
        p2=vert((int)tet(a,0),span(1,3));//get other 3 vertices
		...
		P3 = p3 * r_core;
		P4 = p4 * r_core;
		V = rho_pc*rho * abs(det(p)) / 6.;//determinant of vertices from lit search
		VV = rho_pc*rho * abs(det(P)) / 6;
		I1 = VV * (pow(P1(0, 1), 2) + P1(0, 1)*P2(0, 1) + pow(P2(0, 1), 2) + P1(0, 1)*P3(0, 1) + P2(0, 1)*P3(0, 1) + pow(P3(0, 1), 2) +
			pow(P1(0, 2), 2) + P1(0, 2)*P2(0, 2) + pow(P2(0, 2), 2) + P1(0, 2)*P3(0, 2) + P2(0, 2)*P3(0, 2) + pow(P3(0, 2), 2) +
			P1(0, 1)*P4(0, 1) + P2(0, 1)*P4(0, 1) + P3(0, 1)*P4(0, 1) + pow(P4(0, 1), 2) +
			P1(0, 2)*P4(0, 2) + P2(0, 2)*P4(0, 2) + P3(0, 2)*P4(0, 2) + pow(P4(0, 2), 2)) / 10.;
		I2 = VV * (pow(P1(0, 0), 2) + P1(0, 0)*P2(0, 0) + pow(P2(0, 0), 2) + P1(0, 0)*P3(0, 0) + P2(0, 0)*P3(0, 0) + pow(P3(0, 0), 2) +
			pow(P1(0, 2), 2) + P1(0, 2)*P2(0, 2) + pow(P2(0, 2), 2) + P1(0, 2)*P3(0, 2) + P2(0, 2)*P3(0, 2) + pow(P3(0, 2), 2) +
			P1(0, 0)*P4(0, 0) + P2(0, 0)*P4(0, 0) + P3(0, 0)*P4(0, 0) + pow(P4(0, 0), 2) +
			P1(0, 2)*P4(0, 2) + P2(0, 2)*P4(0, 2) + P3(0, 2)*P4(0, 2) + pow(P4(0, 2), 2)) / 10.;
		I3 = VV * (pow(P1(0, 1), 2) + P1(0, 1)*P2(0, 1) + pow(P2(0, 1), 2) + P1(0, 1)*P3(0, 1) + P2(0, 1)*P3(0, 1) + pow(P3(0, 1), 2) +
			pow(P1(0, 0), 2) + P1(0, 0)*P2(0, 0) + pow(P2(0, 0), 2) + P1(0, 0)*P3(0, 0) + P2(0, 0)*P3(0, 0) + pow(P3(0, 0), 2) +
			P1(0, 1)*P4(0, 1) + P2(0, 1)*P4(0, 1) + P3(0, 1)*P4(0, 1) + pow(P4(0, 1), 2) +
		...
		(*I)(0, 0) += V * (pow(p1(0, 1), 2) + p1(0, 1)*p2(0, 1) + pow(p2(0, 1), 2) + p1(0, 1)*p3(0, 1) + p2(0, 1)*p3(0, 1) + pow(p3(0, 1), 2) +
			pow(p1(0, 2), 2) + p1(0, 2)*p2(0, 2) + pow(p2(0, 2), 2) + p1(0, 2)*p3(0, 2) + p2(0, 2)*p3(0, 2) + pow(p3(0, 2), 2) +
			p1(0, 1)*p4(0, 1) + p2(0, 1)*p4(0, 1) + p3(0, 1)*p4(0, 1) + pow(p4(0, 1), 2) +
			p1(0, 2)*p4(0, 2) + p2(0, 2)*p4(0, 2) + p3(0, 2)*p4(0, 2) + pow(p4(0, 2), 2)) / 10. - I1 + I1 / rho_pc ;//Ixx
		(*I)(0, 1) += V * (pow(p1(0, 0), 2) + p1(0, 0)*p2(0, 0) + pow(p2(0, 0), 2) + p1(0, 0)*p3(0, 0) + p2(0, 0)*p3(0, 0) + pow(p3(0, 0), 2) +
			pow(p1(0, 2), 2) + p1(0, 2)*p2(0, 2) + pow(p2(0, 2), 2) + p1(0, 2)*p3(0, 2) + p2(0, 2)*p3(0, 2) + pow(p3(0, 2), 2) +
			p1(0, 0)*p4(0, 0) + p2(0, 0)*p4(0, 0) + p3(0, 0)*p4(0, 0) + pow(p4(0, 0), 2) +
			p1(0, 2)*p4(0, 2) + p2(0, 2)*p4(0, 2) + p3(0, 2)*p4(0, 2) + pow(p4(0, 2), 2)) / 10. - I2 + I2 / rho_pc ;//Iyy
		(*I)(0, 2) += V * (pow(p1(0, 1), 2) + p1(0, 1)*p2(0, 1) + pow(p2(0, 1), 2) + p1(0, 1)*p3(0, 1) + p2(0, 1)*p3(0, 1) + pow(p3(0, 1), 2) +
			pow(p1(0, 0), 2) + p1(0, 0)*p2(0, 0) + pow(p2(0, 0), 2) + p1(0, 0)*p3(0, 0) + p2(0, 0)*p3(0, 0) + pow(p3(0, 0), 2) +
			p1(0, 1)*p4(0, 1) + p2(0, 1)*p4(0, 1) + p3(0, 1)*p4(0, 1) + pow(p4(0, 1), 2) +
			p1(0, 0)*p4(0, 0) + p2(0, 0)*p4(0, 0) + p3(0, 0)*p4(0, 0) + pow(p4(0, 0), 2)) / 10. - I3 + I3 / rho_pc ;//Izz
    }
    return;
}
...
void poly_moi_met_core(double rho, string tet_file, string vert_file, mat* I, double rho_pc, double r_core){
    ...
    vert.load(vert_file,csv_ascii);
    vert=vert/1000.;//convert from km to meters
	double V, VV, I1, I2, I3, rho_core, rho_surface;
	mat p1, p2, p3, p4, p, P, P1, P2, P3, P4;
	rho_pc = rho_pc / 100;
	rho_core = rho;
	rho_surface = rho_pc * rho;
	for(int a=0;a<tet.n_rows;a++){
        p1.zeros(1,3);//assuming all tet have vertex at origin
        p2=vert((int)tet(a,0),span(1,3));//get other 3 vertices
        p3=vert((int)tet(a,1),span(1,3));
		...
		P3 = p3 * r_core;
		P4 = p4 * r_core;
		V = rho_surface*abs(det(p)) / 6.;
		VV = rho_surface * abs(det(P)) / 6;
		I1 = VV * (pow(P1(0, 1), 2) + P1(0, 1)*P2(0, 1) + pow(P2(0, 1), 2) + P1(0, 1)*P3(0, 1) + P2(0, 1)*P3(0, 1) + pow(P3(0, 1), 2) +
			pow(P1(0, 2), 2) + P1(0, 2)*P2(0, 2) + pow(P2(0, 2), 2) + P1(0, 2)*P3(0, 2) + P2(0, 2)*P3(0, 2) + pow(P3(0, 2), 2) +
			P1(0, 1)*P4(0, 1) + P2(0, 1)*P4(0, 1) + P3(0, 1)*P4(0, 1) + pow(P4(0, 1), 2) +
			P1(0, 2)*P4(0, 2) + P2(0, 2)*P4(0, 2) + P3(0, 2)*P4(0, 2) + pow(P4(0, 2), 2)) / 10.;
...
void ell_mass_params_met_linear(double order, double order_body, double rho, double rho_pc, double a, double b, double c, mat* I, cube* T) {
	...
	double rho1 = rho;
	double rho2 = rho*rho_pc/100;
	cout << "rho1 " << rho1 << endl;
	cout << "rho2 " << rho2 << endl;
	(*T).zeros(order + 1, order + 1, order + 1);//set up inertia integrals and moments
	(*T)(0, 0, 0) = (b*c*M_PI*(3 * rho2 - 3 * rho1 + 4 * a*rho1)) / 3;
	(*I)(0, 0) = 2 * c*pow(b, 3)*M_PI*((5 * rho2 - 5 * rho1 + 6 * a*rho1) / 45) + 2 * b*pow(c, 3)*M_PI*((5 * rho2 - 5 * rho1 + 6 * a*rho1) / 45);
	(*I)(0, 1) = 2 * c*b*pow(a, 2)*M_PI*((5 * rho2 - 5 * rho1 + 6 * a*rho1) / 45) + 2 * b*pow(c, 3)*M_PI*((5 * rho2 - 5 * rho1 + 6 * a*rho1) / 45);
	(*I)(0, 2) = 2 * c*b*pow(a, 2)*M_PI*((5 * rho2 - 5 * rho1 + 6 * a*rho1) / 45) + 2 * c*pow(b, 3)*M_PI*((5 * rho2 - 5 * rho1 + 6 * a*rho1) / 45);

	//conditionals to calculate inertia integrals based on closed form solution. Closed form shows ellipsoids have 0's for odd ordered inertia integrals
	if (order_body > 0) {
		(*T)(2, 0, 0) = (2 * pow(a, 2) * b*c*M_PI*(5 * rho2 - 5 * rho1 + 6 * a*rho1)) / 45;
		(*T)(0, 2, 0) = (2 * pow(b, 3) * c*M_PI*(5 * rho2 - 5 * rho1 + 6 * a*rho1)) / 45;
		(*T)(0, 0, 2) = (2 * b*pow(c, 3) * M_PI*(5 * rho2 - 5 * rho1 + 6 * a*rho1)) / 45;
		if (order_body > 3) {
			(*T)(4, 0, 0) = (pow(a, 4) * b*c*M_PI*(7 * rho2 - 7 * rho1 + 8 * a*rho1)) / 70;
			(*T)(0, 4, 0) = (pow(b, 5) * c*M_PI*(7 * rho2 - 7 * rho1 + 8 * a*rho1)) / 70;
			(*T)(0, 0, 4) = (b*pow(c,5) * M_PI*(7 * rho2 - 7 * rho1 + 8 * a*rho1)) / 70;
			(*T)(2, 2, 0) = (pow(a, 2) * pow(b, 3) * c*M_PI*(7 * rho2 - 7 * rho1 + 8 * a*rho1)) / 210;
			(*T)(0, 2, 2) = (pow(b, 3) * pow(c, 3) * M_PI*(7 * rho2 - 7 * rho1 + 8 * a*rho1)) / 210;
			(*T)(2, 0, 2) = (pow(a, 2) * b*pow(c, 3) * M_PI*(7 * rho2 - 7 * rho1 + 8 * a*rho1)) / 210;
		}
	}
	return;
}
...
void ell_mass_params_met_core(double order, double order_body, double rho, double rho_pc, double r_core, double a, double b, double c, mat* I, cube* T) {
	...
	double rho1 = rho; // Core density
	double rho2 = rho * rho_pc/100; // Surface layer density
	(*T).zeros(order + 1, order + 1, order + 1);//set up inertia integrals and moments
	(*T)(0, 0, 0) = (4 * M_PI*a*b*c*(rho2 + pow(r_core, 3) * rho1 - pow(r_core, 3) * rho2)) / 3;
	(*I)(0, 0) = (4 * M_PI*a*pow(b, 3) * c*(rho2 + pow(r_core, 5) * rho1 - pow(r_core, 5) * rho2)) / 15 + (4 * M_PI*a*b*pow(c, 3) * (rho2 + pow(r_core, 5) * rho1 - pow(r_core, 5) * rho2)) / 15;
	(*I)(0, 1) = (4 * M_PI*pow(a, 3) * b*c*(rho2 + pow(r_core, 5) * rho1 - pow(r_core, 5) * rho2)) / 15 + (4 * M_PI*a*b*pow(c, 3) * (rho2 + pow(r_core, 5) * rho1 - pow(r_core, 5) * rho2)) / 15;
	(*I)(0, 2) = (4 * M_PI*pow(a, 3) * b*c*(rho2 + pow(r_core, 5) * rho1 - pow(r_core, 5) * rho2)) / 15 + (4 * M_PI*a*pow(b, 3) * c*(rho2 + pow(r_core, 5) * rho1 - pow(r_core, 5) * rho2)) / 15;

	//conditionals to calculate inertia integrals based on closed form solution. Closed form shows ellipsoids have 0's for odd ordered inertia integrals
	if (order_body > 0) {
		(*T)(2, 0, 0) = (4 * M_PI*pow(a, 3) * b*c*(rho2 + pow(r_core, 5) * rho1 - pow(r_core, 5) * rho2)) / 15;
		(*T)(0, 2, 0) = (4 * M_PI*a*pow(b, 3) * c*(rho2 + pow(r_core, 5) * rho1 - pow(r_core, 5) * rho2)) / 15;
		(*T)(0, 0, 2) = (4 * M_PI*a*b*pow(c, 3) * (rho2 + pow(r_core, 5) * rho1 - pow(r_core, 5) * rho2)) / 15;
		if (order_body > 3) {
			(*T)(4, 0, 0) = (4 * M_PI*pow(a, 5) * b*c*(rho2 + pow(r_core, 7) * rho1 - pow(r_core, 7) * rho2)) / 35;
			(*T)(0, 4, 0) = (4 * M_PI*a*pow(b, 5) * c*(rho2 + pow(r_core, 7) * rho1 - pow(r_core, 7) * rho2)) / 35;
			(*T)(0, 0, 4) = (4 * M_PI*a*b*pow(c, 5) * (rho2 + pow(r_core, 7) * rho1 - pow(r_core, 7) * rho2)) / 35;
			(*T)(2, 2, 0) = (4 * M_PI*pow(a, 3) * pow(b, 3) * c*(rho2 + pow(r_core, 7) * rho1 - pow(r_core, 7) * rho2)) / 105;
			(*T)(0, 2, 2) = (4 * M_PI*a*pow(b, 3) * pow(c, 3) * (rho2 + pow(r_core, 7) * rho1 - pow(r_core, 7) * rho2)) / 105;
			(*T)(2, 0, 2) = (4 * M_PI*pow(a, 3) * b*pow(c, 3) * (rho2 + pow(r_core, 7) * rho1 - pow(r_core, 7) * rho2)) / 105;
		}
	}
	return;
}
...
void rk87(double t0, double tf, mat x0, double rel_tol, parameters inputs, mat* tout, mat* xout, std::function<mat(mat,mat, parameters)> ode){
	...
	system("rmdir /s /q output_t 2>nul & mkdir output_t");
    // system("if [ -d output_x ]; then rm -rf output_x; fi; mkdir output_x");
	system("rmdir /s /q output_t 2>nul & mkdir output_x");
...
double rk4_stack(double t0, double tf, mat x0, double h, int a_shape, parameters inputs, mat* tout, mat* xout, mat* hout, mat*sunout, std::function<mat(mat,mat, parameters)> ode){
	//system commands which remove old integrated files and create new output directories
    system("rmdir /s /q output_t 2>nul & mkdir output_t");
	system("rmdir /s /q output_x 2>nul & mkdir output_x");
    system("rmdir /s /q output_h 2>nul & mkdir output_h");
    system("rmdir /s /q output_sun 2>nul & mkdir output_sun");
    ofstream tfile("output_t/t_out.bin", std::ios::binary);
	ofstream xfile("output_x/x_out.bin", std::ios::binary);
	...
	int files = 1;
	mat t_temp, y_temp;
	// mat current_time, prev_time, time_elapsed;
	// mat prev_x, prev_y, prev_z, current_x, current_y, current_z;
	// double time_elapsed_double, diff_x, diff_y, diff_z;
	// int rev = 0;
    ...
    while(t(0,0)<tf){
		...
		r = sqrt(pow(y_temp(0),2)+pow(y_temp(1),2)+pow(y_temp(2),2));
		v = sqrt(pow(y_temp(3), 2) + pow(y_temp(4), 2) + pow(y_temp(5), 2));
		T = 2 * M_PI*r / v;
		periods.push_back(T);
		...
		if (count==(int(86400/h)-1)){//this is the print out to the binary files (when the row is the last one! --> full)
			for (int indr = 0; indr < (*xout).n_rows; indr++) {
				tfile.write((char*) &((*tout)(indr, 0)), sizeof(double));
				for (int indc = 0; indc < (*xout).n_cols; indc++) {
					xfile.write((char*) &((*xout)(indr, indc)), sizeof(double));
	...
	// Calculate the mean of the `periods` vector
	double period = sum / periods.size();
...
double ABM(double t0, double tf, mat x0, double h, int a_shape, parameters inputs, mat* tout, mat* xout, mat* hout, mat*sunout, std::function<mat(mat,mat, parameters)> ode){
    ...
    while(t(0,0)<tf){
        if(t(0,0)<=3*h){
			...
			current_z = y_temp(2);
			if (a_shape == 1) {
				if ((prev_x(0, 0)*current_x(0, 0)) < 0) {
					rev = rev + 1;
					if (rev > 3) {
						time_elapsed = current_time - prev_time;
						time_elapsed_double = time_elapsed(0, 0);
						periods.push_back(time_elapsed_double);
			...
			if (a_shape == 2) {
				diff_x = abs(prev_x(0, 0) - current_x(0, 0));
				diff_y = abs(prev_y(0, 0) - current_y(0, 0));
				diff_z = abs(prev_z(0, 0) - current_z(0, 0));
				cout << "diff_x " << diff_x << endl;
				cout << "diff_y " << diff_y << endl;
				cout << "diff_z " << diff_z << endl;
				if ((diff_x < 0.01) && (diff_y < 0.05) && (diff_z < 0.0001) && (count > 1)) {
					time_elapsed = current_time - prev_time;
					time_elapsed_double = time_elapsed(0, 0);
					if (time_elapsed_double > 30000) {
						cout << "diff_x " << diff_x << endl;
						cout << "diff_y " << diff_y << endl;
						cout << "diff_z " << diff_z << endl;
						cout << count << endl;
			...
			r = sqrt(pow(y_0(0), 2) + pow(y_0(1), 2) + pow(y_0(2), 2));
			v = sqrt(pow(y_0(3), 2) + pow(y_0(4), 2) + pow(y_0(5), 2));
			T = 2 * M_PI*r / v;
			periods.push_back(T);
			...
			if (a_shape == 2) {
				...
				diff_y = abs(prev_y(0, 0) - current_y(0, 0));
				diff_z = abs(prev_z(0, 0) - current_z(0, 0));
				if ((diff_x < 0.01) && (diff_y < 0.02) && (diff_z < 0.0001) && (count > 1)) {
					time_elapsed = current_time - prev_time;
					time_elapsed_double = time_elapsed(0, 0);
					if (time_elapsed_double > 30000) {
						cout << "diff_x " << diff_x << endl;
...
double LGVI_integ(double t0, double tf, mat x0, double h, parameters inputs, mat* tout, mat* xout){
	...
	mat PERIODS = periods;
	PERIODS.save("PERIODS.csv", csv_ascii);
	// Calculate the mean of the `periods` vector
	double period = sum / periods.size();
...
mat hou_ode(mat x, mat t, parameters inputs){
    ...
    //because of use of C and Cd must account for partial of rotated inertia matrix with respect to time
